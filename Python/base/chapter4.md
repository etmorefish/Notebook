...
- **匿名函数**：一种不需要定义函数名的简化函数，使用 `lambda` 关键字创建。
- **应用场景**：在需要简单功能的地方临时创建函数，尤其适用于简单的一次性操作。
...
- **参数**：可以有多个，也可以没有
- **表达式**：`lambda` 函数只能有一个表达式，表达式的结果即为返回值
...
1. **用于 `map`、`filter` 等函数**
   ...
   - 匿名函数经常用于 `map()`、`filter()` 这种需要传递函数对象的地方。
   - `map()`：对序列中的每个元素应用某个函数，并返回新序列
   - `filter()`：筛选出符合条件的元素
...
2. **用于排序中的键函数**
   ...
   - `lambda` 表达式可以用于 `sorted()`、`sort()` 函数中的 `key` 参数来指定排序规则。
...
- 匿名函数只能包含一个表达式，无法写多行复杂逻辑。
- 通常适合小型且简单的操作，复杂逻辑建议用 `def` 定义普通函数。
...
### 1. 概述
递归函数是指在函数内部调用自身的函数。递归的核心思想是将一个大问题分解成更小的子问题，通过不断地自我调用，逐步逼近最终解。递归通常适用于分解性问题，比如斐波那契数列、阶乘计算等。

递归的两个关键组成部分是：
1. **基本情况**（base case）：终止条件，用于防止无限递归。
2. **递归调用**（recursive call）：在函数中调用自身来解决子问题。

---

### 2. 斐波那契数列

斐波那契数列是一组数列，从 0 和 1 开始，每个数都是前两个数之和。具体的数学公式为：
...
- **优点**：代码简洁明了，适用于分治法问题。
- **缺点**：递归层次较深时，容易造成性能问题，甚至导致内存溢出。

---

通过斐波那契数列的示例，可以理解递归的结构和核心思想，掌握在 Python 中如何使用递归方法来解决分解性问题。
